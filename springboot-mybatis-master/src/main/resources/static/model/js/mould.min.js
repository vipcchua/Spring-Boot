...
var _Pjax = function (s, c) {
    if (_m(c).length != undefined || _m(c) == null || _m(c) == undefined) {
        console.error('container is not the right value form.');
        return;
    };
    if (_m(s) == null || _m(s) == undefined) {
        console.error('selector is not the right value form.');
        return;
    };
    if (_m(s).length != undefined) {
        for (var i = 0; i < _m(s).length; i++) {
            if (_m(s)[i].tagName.toUpperCase() != 'A') {
                console.error('selector is not the right value form.');
                return;
            };
            _m(s)[i].onclick = function () {
                pjax(this.href, true);
                return false;
            };
        };
    } else {
        if (_m(s).tagName.toUpperCase() != 'A') {
            console.error('selector is not the right value form.');
            return;
        };
        _m(s).onclick = function () {
            pjax(this.href, true);
            return false;
        };
    };
    ...
    var pjax = function (url, state) {
        ...
        new Promise(function (resolve, reject) {
            ...
            if (_M(_c).child(c).length != undefined || _M(_c).child(c) == null || _M(_c).child(c) == undefined) {
                console.error('container is not the right value form.');
                return;
            };
            if (state) {
                var State = {
                    title: _M(_c).child('title').innerText,
                    url: url
                };
                window.history.pushState(State, State.title, State.url);
            };
            _m(c).innerHTML = _M(_c).child(c).innerHTML;
            _Pjax(s, c);
        });
    };
};
...
method.getElementByAttr = function (attr, param) {          //  根据自定义属性寻找对象.
...
method.parent = function () {
    if (arguments.length == 1) {
        ...
        while (_o != undefined && _o != null && _o.tagName.toUpperCase() != 'BODY') {
            if (_o.hasClass(arguments[0].replace('.', '')) || _o.id == arguments[0].replace('#', '')) {
                arr.push(_o);
            };
            _o = _o.parentNode;
        }
